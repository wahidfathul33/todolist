...
import { supabase, LinkInsert } from '@/lib/supabase'
...
export default function AddLinkModal({ onClose, onSuccess }: AddLinkModalProps) {
  const [form, setForm] = useState<LinkInsert>({ judul: '', url: '', starred: false })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setError('')
    const { error: supaErr } = await supabase.from('links').insert([{ ...form, url }])
    setLoading(false)
    if (supaErr) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-sky-900/20 backdrop-blur-sm">
      <div className="w-full max-w-md md:max-w-2xl lg:max-w-3xl bg-white rounded-3xl shadow-2xl shadow-sky-200/60 animate-bounce-in">
        <div className="bg-gradient-to-r from-sky-400 to-cyan-400 rounded-t-3xl p-5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Link2 className="w-5 h-5 text-white" />
            <h2 className="text-white font-bold text-lg">Link Baru</h2>
          </div>
          <button
            onClick={onClose}
