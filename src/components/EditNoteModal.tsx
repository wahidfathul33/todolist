...
export default function EditNoteModal({ note, onClose, onSuccess }: EditNoteModalProps) {
  const [form, setForm] = useState<NoteUpdate>({ judul: note.judul, catatan: note.catatan })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-violet-900/20 backdrop-blur-sm">
      <div className="w-full max-w-md md:max-w-2xl lg:max-w-3xl bg-white rounded-3xl shadow-2xl shadow-violet-200/60 animate-bounce-in">
