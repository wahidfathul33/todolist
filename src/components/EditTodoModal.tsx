...
export default function EditTodoModal({ todo, onClose, onSuccess }: EditTodoModalProps) {
  const [form, setForm] = useState<TodoUpdate>({
    tanggal: todo.tanggal,
    kegiatan: todo.kegiatan,
      ...
      <div className="w-full max-w-md md:max-w-2xl lg:max-w-3xl bg-white rounded-3xl shadow-2xl shadow-pink-200/60 animate-bounce-in">
        ...
        <div className="bg-gradient-to-r from-violet-400 to-purple-400 rounded-t-3xl p-5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-white font-bold text-lg">Edit Kegiatan</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-violet-400 to-purple-400 text-white font-bold shadow-lg shadow-purple-200 hover:shadow-purple-300 transition-all disabled:opacity-60"
            >
